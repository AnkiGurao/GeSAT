...
ui <- fluidPage(
  ...
  fluidRow(
    column(2,
           tags$img(src = "logo.png", height = "450px", width = "250px", alt = "GeSAT Logo")
    ),
    column(10,
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("genes", "Genes in list", choices = NULL, multiple = TRUE),
      ...
      tabsetPanel(
        ...
        tabPanel(
          "Mixed Model Stability",
            tags$div(
            style = "background:#f8f9fa; padding:12px; border-radius:8px; border:1px solid #e9ecef;",
            tags$h4("Mixed Model Stability — statistical methods"),
            tags$p("This section evaluates candidate reference gene combinations using linear mixed-effects models. The pipeline performs model comparison (LRT), estimates intraclass correlation (ICC), and uses bootstrapped confidence intervals to rank stability."),
            tags$ul(
              tags$li(tags$b("1) Model comparison (Likelihood Ratio Test): "),
                      "Full and reduced mixed models are compared (REML = FALSE). A significant LRT (p ≤ 0.05) indicates systematic gene/group effects — such combinations are excluded from being considered stable."),
              tags$li(tags$b("2) Intraclass Correlation Coefficient (ICC): "),
                      "For combinations with non-significant LRT (p > 0.05), we fit a random-effects model and compute ICC = σ²_sample / (σ²_sample + σ²_residual). A higher ICC indicates more between-sample consistency, which is desirable."),
              tags$li(tags$b("3) Bootstrapped CIs: "),
                      "ICC confidence intervals are obtained via bootstrapping (bootMer). We use the ICC lower bound to rank combinations — higher lower bounds indicate greater reliability."),
              tags$li(tags$b("4) Ranking & selection: "),
                      "Combinations are filtered by LRT (p > 0.05) and ranked by ICC lower bound. The top-ranked combination is suggested as the best reference set."),
              tags$li(tags$b("5) Assumptions & diagnostics: "),
                      "Mixed models assume normal residuals and adequate sample size for bootstrapping. Please inspect model diagnostics (residuals, warnings) if any combinations fail to converge.")
            ),
            tags$p(style = "font-size:90%; color:#555;",
                   "Implementation: models fitted with lme4::lmer, hypothesis tests via anova/add1, ICC bootstrapping via boot::boot.ci (bootMer). See Dai et al. (2013) for details.")
          ),
          ...
          tableOutput("rankedMixedModelTable"),   # <- matches server$output$rankedMixedModelTable
  ...
  ")),
        ...
        ,
        tabPanel("ΔCt",
                 # Ensure required package is loaded (in global.R or server.R)
                 # library(shinycssloaders)
                 ...
                 fluidRow(
                   column(
                     ...
                     wellPanel(
                       h4("Controls"),
                       uiOutput("pair_selector_ui"),  # dynamically generated selector
                       checkboxInput("show_jitter", "Show jitter points", value = TRUE),
                       sliderInput(
                         "annotation_y",
                   ...
                   column(
                     ...
                     # Spinner wraps plotlyOutput: shows a nice loader while rendering
                     shinycssloaders::withSpinner(
                       plotly::plotlyOutput("deltaPlotly", height = "700px"),
                       type = 6, color = "#007BFF"
                     ),
                     ...
                     h5("Static fallback"),
                     shinycssloaders::withSpinner(
                       plotOutput("deltaCtPlot", height = "350px"),
                       type = 4, color = "#28A745"
                     )
                   )
                 ),
                 ...
                 fluidRow(
                   column(
                     ...
                     # Spinner for data table (appears while summary loads)
                     shinycssloaders::withSpinner(
        ...
        tabPanel("Cumulative Ranking",
                 fluidRow(
                   ...
                   tags$div(class = "alert alert-warning", style = "margin-bottom: 12px;",
                            tags$strong("Note: NF computation rule —"),
                            tags$p(
                              "Normalization Factor (NF) will be computed automatically only if the top-N genes",
                              "recommended by geNorm (using the V cutoff) exactly match the top-N genes from the",
                              "combined (cumulative) ranking. If they do not match, NF computation is disabled",
                              "and you will be prompted to inspect the rankings or select genes manually."
                            ),
                            tags$p(
                              "This rule is deliberate: geNorm's pairwise variation (V) assumes the NF is built",
                              "from geNorm's own ranked genes. When cumulative and geNorm rankings coincide, the",
                              "recommendation is consistent and NF may be used safely."
                            ),
                            tags$hr(style = "margin: 8px 0;"),
                            # Dynamic status line rendered from server: output$icg_match_status
                            verbatimTextOutput("icg_match_status")
                   ), br(),
                     column(
                     width = 6,
                     h4("Download Plot"),
                 ...
                 ## --- NEW: Controls + NF compute panel ------------------------------------
                 fluidRow(
                   column(
                     width = 5,
                     wellPanel(
                       h4("Use GeNorm suggestion to compute Normalization Factor (NF)"),
                       p("This will use the genes suggested in the GeNorm tab (top-N by geNorm rank)."),
                       # show the genes that would be used
                       verbatimTextOutput("cumulative_selected_icg_text"),
                       # show whether geNorm top-n and cumulative top-n match
                       verbatimTextOutput("icg_match_status"),
                       # compute button (guarded on server by icg_sets_match())
                       actionButton("compute_icg", "Compute NF & normalize data", class = "btn-primary"),
                       tags$br(), tags$br(),
                       tags$small("NF is computed as geometric mean of relative quantities (RQ = 2^-Ct) across selected ICGs.")
                     )
                   ),
                   column(
                     width = 7,
                     wellPanel(
                       h4("NF summary (by group)"),
                       verbatimTextOutput("nf_group_summary_text"),
                       tags$small("After computing, a per-sample NF table, NF plot, and normalized data preview are shown below.")
                     )
                   )
                 ),
                 br(),
                 ...
                 h4("Normalization Factor (per-sample)"),
                 DT::dataTableOutput("nf_table"),
                 plotOutput("nf_plot", height = "300px"),
                 downloadButton("download_nf_table", "Download NF table", class = "btn-success")
        )
  ...
  tags$footer(
    ...
    HTML(
      "GeSAT | Developed by Ankita Gurao (PhD) at the Buffalo Genomics Lab, led by Dr. Ranjit Singh Kataria & Dr. Mahesh Shivanand Dige, ICAR-National Bureau of Animal Genetic Resources | 2025<br><br>
    ...
    <b>ctrlGene:</b> Zhong S (2019). ctrlGene: Assess the Stability of Candidate Housekeeping Genes. R package version 1.0.1, https://CRAN.R-project.org/package=ctrlGene.
      "
    )
  )
)
...
# recommended_icg_info: inspect pairwise_variation() and pick smallest i with V < 0.15
recommended_icg_info <- reactive({
  req(pairwise_variation())        # requires the V table
  Vdf <- pairwise_variation()
  if (nrow(Vdf) == 0) {
    return(list(rec = NA_integer_, reason = "Not enough genes to compute pairwise variation"))
  }
  
  # V labels are like "V2/3", "V3/4" etc. We will extract the left value (i) and the numeric V.
  # If Value < 0.15 pick the smallest i
  Vdf <- Vdf %>%
    mutate(
      i = as.integer(sub("^V([0-9]+).*$", "\\1", Variation)),
      Value = as.numeric(Value)
    ) %>%
    arrange(i)
  
  # find first where V < 0.15
  idx <- which(!is.na(Vdf$Value) & Vdf$Value < 0.15)
  if (length(idx) == 0) {
    return(list(rec = NA_integer_, reason = "No V < 0.15 found"))
  }
  
  chosen_i <- Vdf$i[idx[1]]
  # number of genes = i+1 (because V_i/(i+1) tests whether adding the (i+1)th gene is necessary)
  n_genes <- chosen_i + 1L
  list(rec = as.integer(n_genes), reason = paste0("First V <", 0.15, " at V", chosen_i, "/", chosen_i+1))
})
...
output$genorm_plot <- renderPlot({
  ...
  axis(1, at = 1:nrow(plot_data), labels = FALSE)
  text(
    x = 1:nrow(plot_data), 
    y = par("usr")[3] - 0.02 * diff(par("usr")[3:4]), # move labels slightly below axis
    labels = plot_data$Target, 
...
# Step 6: Plot ΔCt
  ...
  output$download_delta_ct_plot <- downloadHandler(
    ...
    content = function(file) {
      # show progress while plotting + saving
      withProgress(message = "Rendering and saving ΔCt plot", value = 0, {
        incProgress(0.2, detail = "Preparing data...")
        data <- processed_data()
        incProgress(0.6, detail = "Rendering plot (ggsave)...")
        ggsave(file,
               plot = plot_delta_ct(data$delta_ct_long, data$summary_stats),
  ...
  #STEP6.2:
  # Reactive ggplot builder used by both static and interactive outputs
  plot_delta_ct_gg <- function(delta_ct_long, summary_stats, annotation_y = 12, show_jitter = TRUE, selected_pair = "All") {
    # optionally filter to a single pair
    plot_data <- delta_ct_long
    if (!is.null(selected_pair) && selected_pair != "All") {
      plot_data <- plot_data %>% filter(Gene_Pair == selected_pair)
    }
    
    group_levels <- unique(plot_data$Group)
    color_palette <- hue_pal()(max(1, length(group_levels)))
    
    p <- ggplot(plot_data, aes(x = Gene_Pair, y = `ΔCt`, fill = Group, text = paste0(
      "Gene Pair: ", Gene_Pair,
      "\nGroup: ", Group,
      ifelse(!is.null(plot_data$Sample), paste0("\nSample: ", plot_data$Sample), ""),
      "\nΔCt: ", round(`ΔCt`, 3)
    ))) +
      geom_boxplot(outlier.shape = NA, alpha = 0.6, position = position_dodge(width = 0.8)) +
      { if (show_jitter) geom_jitter(position = position_jitter(width = 0.2, height = 0),
                                     alpha = 0.6, size = 1.8, aes(text = paste0(
                                       "Gene Pair: ", Gene_Pair,
                                       "\nGroup: ", Group,
                                       ifelse(!is.null(plot_data$Sample), paste0("\nSample: ", plot_data$Sample), ""),
                                       "\nΔCt: ", round(`ΔCt`, 3)
      ...
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
      labs(x = "Gene Pair", y = "\u0394Ct") 
    
    # annotation: place per First_Gene if showing all pairs; otherwise just annotate selected pair
    if (nrow(summary_stats) > 0) {
      # build annotation data -- matching x positions requires converting factors consistently
      # When using a subset (selected_pair != "All") the First_Gene label might not match x - we fallback to adding a caption
      if (selected_pair == "All") {
        ann_df <- summary_stats %>%
          # For annotation to work with Genepair x axis we annotate at the First_Gene name
          # summary_stats$First_Gene should match the gene names used in Gene_Pair first part of your data
          mutate(x = First_Gene,
                 label = paste0(First_Gene,
                                "\nx̄ΔCt: ", round(Mean_ΔCt, 2),
                                "\nMean SD: ", round(Mean_of_StdDev, 2)))
        p <- p + geom_text(data = ann_df,
                           aes(x = x, y = annotation_y, label = label),
                           inherit.aes = FALSE, size = 3.5, hjust = 0, color = "black")
      } else {
        # simpler caption if single pair is selected
        p <- p + labs(caption = paste0("Selected pair: ", selected_pair))
      }
    }
  ...
  output$deltaPlotly <- plotly::renderPlotly({
    req(processed_data())
    
    annotation_y <- input$annotation_y %||% 12
    show_jitter <- if (is.null(input$show_jitter)) TRUE else input$show_jitter
    selected_pair <- if (is.null(input$select_pair) || input$select_pair == "") "All" else input$select_pair
    
    delta_ct_long <- processed_data()$delta_ct_long
    summary_stats <- processed_data()$summary_stats
    
    # create ggplot
    p <- plot_delta_ct_gg(delta_ct_long, summary_stats, annotation_y = annotation_y, 
                          show_jitter = show_jitter, selected_pair = selected_pair)
    
    # Convert to plotly and tweak layout/tooltip
    ggplotly(p, tooltip = "text") %>%
      layout(boxmode = "group",
             legend = list(orientation = "h", x = 0.1, y = 1.05),
             margin = list(b = 200)) %>%
      config(displayModeBar = TRUE, modeBarButtonsToAdd = list("toggleSpikelines"))
  })
  
  # Static fallback ggplot (keeps server-side PNG download consistent)
  ...
  processed_data <- reactive({
    ...
    withProgress(message = "Calculating ΔCt and ranking genes", value = 0, {
      # Step 1: Preprocess
      incProgress(0.10, detail = "Preprocessing Ct table...")
      long_data <- deltact_prep(ct_data())
      
      # Step 2: Compute pairwise ΔCt (likely the heaviest step)
      incProgress(0.35, detail = "Computing pairwise ΔCt for selected genes...")
      delta_ct_df <- compute_delta_ct(long_data, input$genes)
      
      # Step 3: Summarize & rank
      incProgress(0.40, detail = "Summarizing ΔCt and ranking genes...")
      res <- summarize_delta_ct(delta_ct_df)
      
      # Step 4: finalizing
      incProgress(0.15, detail = "Finalizing results...")
      # return the result
      res
    })
  })
...
# Step 8: Output Summary Statistics Table
  ...
  output$download_delta_ct_summary <- downloadHandler(
    ...
    content = function(file) {
      withProgress(message = "Exporting ΔCt summary CSV", value = 0, {
        incProgress(0.6, detail = "Preparing summary data...")
...
########## --- Standard Curve Plot ---
  ...
  # Guarded NF computation + normalized data
  # Place this inside server()
  
  # 1) Event: compute NF only when user clicks and guard conditions pass
  computed_nf <- eventReactive(input$compute_icg, {
    req(ct_data())                 # Ct table must exist
    # check recommendation / matching status
    info <- tryCatch(recommended_icg_info(), error = function(e) NULL)
    if (is.null(info) || is.na(info$rec)) {
      showNotification("No geNorm recommendation available; cannot compute NF.", type = "error", duration = 6)
      return(NULL)
    }
    
    # ensure cumulative / geNorm top-n sets match
    if (!icg_sets_match()) {
      showNotification("Cannot compute NF: geNorm top-n genes do NOT match cumulative top-n. Please inspect both rankings or select ICGs manually.", type = "error", duration = 8)
      return(NULL)
    }
    
    # get the genes (they should be identical sets due to the check)
    genes <- selected_icg_genes_from_cumulative()
    if (is.null(genes) || length(genes) == 0) {
      showNotification("No ICGs selected", type = "error", duration = 6)
      return(NULL)
    }
    
    # safety: ensure gene columns exist in ct table
    missing_genes <- setdiff(genes, colnames(ct_data()))
    if (length(missing_genes) > 0) {
      showNotification(paste("Missing gene columns:", paste(missing_genes, collapse = ", ")), type = "error", duration = 8)
      return(NULL)
    }
    
    # compute RQ = 2^-Ct and geometric mean per sample (NF)
    ct_df <- ct_data()
    # convert selected gene Ct columns to numeric matrix (preserve row order)
    rq_mat <- tryCatch({
      mat <- as.matrix(sapply(ct_df[, genes, drop = FALSE], as.numeric))
      2 ^ (- mat)
    }, error = function(e) {
      showNotification(paste("Error computing RQ:", e$message), type = "error")
      return(NULL)
    })
    if (is.null(rq_mat)) return(NULL)
    
    # geometric mean per sample (ignore NAs per sample)
    gm <- apply(rq_mat, 1, function(x) {
      x <- x[!is.na(x)]
      if (length(x) == 0) return(NA_real_)
      exp(mean(log(x)))
    })
    
    nf_tbl <- tibble::tibble(Sample = ct_df$Sample, Group = ct_df$Group, NF = gm)
    attr(nf_tbl, "ICGs") <- genes
    
    showNotification(paste0("NF computed using genes: ", paste(genes, collapse = ", ")), type = "message", duration = 5)
    nf_tbl
  }, ignoreNULL = FALSE)   # ignoreNULL FALSE ensures reactive exists before click (but returns NULL until clicked)
  
  # 2) normalized data reactive: RQ_all / NF per sample
  normalized_data <- reactive({
    nf <- computed_nf()
    req(nf, ct_data())
    
    ct_df <- ct_data()
    # gene columns = everything except Sample and Group
    gene_cols <- setdiff(colnames(ct_df), c("Sample", "Group"))
    if (length(gene_cols) == 0) return(NULL)
    
    # compute RQ for all genes (rows = samples, cols = genes)
    rq_all <- as.data.frame(sapply(ct_df[, gene_cols, drop = FALSE], function(x) 2 ^ (- as.numeric(x))))
    # divide each row by NF
    norm_mat <- sweep(as.matrix(rq_all), 1, nf$NF, FUN = "/")
    norm_df <- as.data.frame(norm_mat)
    # reattach Sample & Group
    norm_df <- tibble::tibble(Sample = ct_df$Sample, Group = ct_df$Group, norm_df)
    attr(norm_df, "ICGs") <- attr(nf, "ICGs")
    norm_df
  })
  
  # 3) NF table (per-sample) shown with DT
  output$nf_table <- DT::renderDataTable({
    req(computed_nf())
    DT::datatable(computed_nf(), options = list(pageLength = 10, autoWidth = TRUE), rownames = FALSE)
  })
  
  # 4) NF group summary (text)
  output$nf_group_summary_text <- renderText({
    req(computed_nf())
    nf <- computed_nf()
    summary_df <- nf %>%
      dplyr::group_by(Group) %>%
      dplyr::summarise(Mean_NF = mean(NF, na.rm = TRUE), SD_NF = sd(NF, na.rm = TRUE), n = n(), .groups = "drop")
    
    paste0(
      "NF by Group (mean ± SD):\n",
      paste0(summary_df$Group, ": ", formatC(summary_df$Mean_NF, digits = 4, format = "g"),
             " ± ", formatC(summary_df$SD_NF, digits = 4, format = "g"),
             " (n=", summary_df$n, ")", collapse = "\n")
    )
  })
  
  # 5) NF plot (per-sample bar plot)
  output$nf_plot <- renderPlot({
    req(computed_nf())
    nf <- computed_nf()
    # arrange for nicer plot
    nf <- nf %>% arrange(Group, Sample)
    # ensure Sample is a factor in order
    nf$Sample <- factor(nf$Sample, levels = unique(nf$Sample))
    ggplot2::ggplot(nf, aes(x = Sample, y = NF, fill = Group)) +
      ggplot2::geom_col(position = "dodge") +
      ggplot2::labs(title = sprintf("Normalization Factor per sample (based on %d ICGs)", length(attr(nf, "ICGs"))),
                    x = "Sample", y = "NF (geometric mean of RQ)") +
      ggplot2::theme_minimal() +
      ggplot2::theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
  })
  
  # 7) Download handlers for NF and normalized data
  output$download_nf_table <- downloadHandler(
    filename = function() paste0("Normalization_Factors_", Sys.Date(), ".csv"),
    content = function(file) {
      req(computed_nf())
      write.csv(computed_nf(), file, row.names = FALSE)
    }
  )
  

  # 8) Optional: expose selected genes text if not already defined
  output$cumulative_selected_icg_text <- renderText({
    # try both reactives depending on what you already defined
    genes <- NULL
    try(genes <- selected_icg_genes_from_cumulative(), silent = TRUE)
    if (is.null(genes) || length(genes) == 0) {
      "Selected ICGs: (none)"
    } else {
      paste0("Selected ICGs (top ", length(genes), "): ", paste(genes, collapse = ", "))
    }
  })
  
  # --- Helper reactives: get top-n sets (use geNorm rec to choose n) ---
  rec_icg_n <- reactive({
    info <- recommended_icg_info()
    # If info is NULL or rec is NA, fallback to 2
    if (is.null(info) || is.na(info$rec)) return(2L)
    as.integer(info$rec)
  })
  
  selected_icg_genes_from_genorm <- reactive({
    req(geNorm_table(), rec_icg_n())
    n <- rec_icg_n()
    gn <- geNorm_table()
    gn %>% arrange(Rank) %>% slice_head(n = n) %>% pull(Target)
  })
  
  selected_icg_genes_from_cumulative <- reactive({
    req(cumulative_data(), rec_icg_n())
    n <- rec_icg_n()
    cum <- cumulative_data()
    if (!"Cumulative_Geomean_Rank" %in% colnames(cum)) {
      stop("cumulative_data() must contain 'Cumulative_Geomean_Rank' column")
    }
    cum %>% arrange(Cumulative_Geomean_Rank) %>% slice_head(n = n) %>% pull(Gene)
  })
  
  # --- Check whether the two sets match exactly (order-insensitive equality) ---
  icg_sets_match <- reactive({
    req(selected_icg_genes_from_genorm(), selected_icg_genes_from_cumulative())
    a <- selected_icg_genes_from_genorm()
    b <- selected_icg_genes_from_cumulative()
    setequal(a, b)
  })
  
  # Guarded NF computation: only runs when geNorm and cumulative top-n agree
  computed_nf <- eventReactive(input$compute_icg, {
    req(ct_data())
    info <- recommended_icg_info()
    if (is.null(info) || is.na(info$rec)) {
      showNotification("No geNorm recommendation available; cannot compute NF.", type = "error")
      return(NULL)
    }
    
    if (!icg_sets_match()) {
      showNotification("Cannot compute NF: geNorm top-n genes do NOT match cumulative top-n. Please inspect both rankings or choose genes manually.", type = "error", duration = 8)
      return(NULL)
    }
    
    genes <- selected_icg_genes_from_cumulative()  # sets are equal, so either reactive is fine
    missing_genes <- setdiff(genes, colnames(ct_data()))
    if (length(missing_genes) > 0) {
      showNotification(paste("Missing gene columns:", paste(missing_genes, collapse = ", ")), type = "error")
      return(NULL)
    }
    
    ct_df <- ct_data()
    rq_mat <- 2 ^ (- as.matrix(ct_df[, genes, drop = FALSE]))
    gm <- apply(rq_mat, 1, function(x) {
      x <- x[!is.na(x)]
      if (length(x) == 0) return(NA_real_)
      exp(mean(log(x)))
    ...
    nf_tbl <- tibble::tibble(Sample = ct_df$Sample, Group = ct_df$Group, NF = gm)
    attr(nf_tbl, "ICGs") <- genes
    showNotification(paste0("NF computed using genes: ", paste(genes, collapse = ", ")), type = "message")
    nf_tbl
  }, ignoreNULL = FALSE)
  
  
  # --- Expose a status text for UI to show match / mismatch and reason ---
  output$icg_match_status <- renderText({
    info <- recommended_icg_info()
    if (is.null(info)) return("geNorm recommendation not ready.")
    if (is.na(info$rec)) {
      return(paste0("geNorm did not produce a recommendation: ", info$reason))
    }
    if (icg_sets_match()) {
      paste0("OK: geNorm recommends n = ", rec_icg_n(), 
             " and cumulative top-", rec_icg_n(), " matches geNorm's top-", rec_icg_n(),
             ". NF computation is allowed.")
    } else {
      paste0("MISMATCH: geNorm recommends n = ", rec_icg_n(), 
             " but cumulative top-", rec_icg_n(), " genes DO NOT match geNorm's top-", rec_icg_n(),
             ". NF computation will be blocked. Inspect both rankings or select genes manually.")
    }
  })
