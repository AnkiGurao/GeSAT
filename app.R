...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      # ---------------------------------------------------------
      # GLOBAL IMAGE EXPORT SETTINGS
      # ---------------------------------------------------------
      tags$h4("Export Settings"),
      ...
      helpText(
        "Increase width and height for figures containing many genes, ",
        "facets, or panels. Default resolution: 300 DPI."
      )
    ),
...
server <- function(input, output, session) {
  ...
  # =========================================================
  # GLOBAL EXPORT SETTINGS (used by every download handler below)
  # =========================================================
  
  save_plot <- function(plot, file, bg = "white") {
  ...
  save_base_plot <- function(file, plot_function) {
    ...
    width_px <- round(input$export_width * as.numeric(input$export_dpi))
    height_px <- round(input$export_height * as.numeric(input$export_dpi))
  ...
  output$bestkeeper_plot <- renderPlot({
    ...
    output$download_bestkeeper_plot <- downloadHandler(
      ...
      content = function(file) {
        ...
        p <- ggplot(df, aes(x = Gene, y = `coeff. of corr. [r]`, fill = significance)) +
          geom_bar(stat = "identity") +
          geom_text(aes(label = ifelse(significance == "Not significant", "ns", "")),
                    vjust = -0.5, size = 4) +
          scale_fill_manual(values = c("Significant" = "green", "Not significant" = "black")) +
  ...
  # Download handlers (updated to use global export settings)
  ...
  # Mixed Model Individual Gene Ranking
  ...
  # Reusable reactive so both the on-screen plot and the download use the same object
  icc_rank_plot <- reactive({
    req(ranked_mixed_model_results())
  ...
  output$download_icc_rank_plot <- downloadHandler(
    ...
    content = function(file) {
      ...
      save_plot(plot = icc_rank_plot(), file = file)
    }
  )
  ...
  output$download_delta_ct_plot <- downloadHandler(
    ...
    content = function(file) {
      ...
      withProgress(message = "Rendering and saving ΔCt plot", value = 0, {
        ...
        data <- processed_data()
        incProgress(0.6, detail = "Rendering plot...")
        p <- plot_delta_ct(data$delta_ct_long, data$summary_stats)
        save_plot(plot = p, file = file)
  ...
  output$download_std_curve_plot <- downloadHandler(
    ...
    content = function(file) {
      ...
      save_plot(plot = final_plot, file = file)
    }
  )
  ...
  output$download_ct_dist_plot <- downloadHandler(
    ...
    content = function(file) {
      ...
      save_plot(plot = p, file = file, bg = "white")
    }
  )
  ...
  # normalized data reactive: RQ_all / NF per sample
  ...
  # Download handlers for NF and normalized data
  ...
  # Cumulative ranking plot (reactive, reused by on-screen render and download)
  cumulative_plot_obj <- reactive({
    req(cumulative_data())
  ...
  # Download handler for cumulative ranking plot
  output$download_cumulative_plot <- downloadHandler(
    ...
    content = function(file) {
      save_plot(plot = cumulative_plot_obj(), file = file)
    }
  )
  
  # Download handler for detailed table
  ...
  # Download handler for raw ranked data
